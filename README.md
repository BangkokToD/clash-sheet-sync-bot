...
- Прогноз ЛВК по фактической текущей войне и потенциальной силе всех оставшихся
  соперников.
- Кнопочный ввод будущего расписания ЛВК администратором без OCR и
  неофициальных API.
...
- «Рассылка всем» принимает одно текстовое сообщение, показывает аудиторию и
  предпросмотр, затем требует явного подтверждения. Premium/custom emoji сохраняются
  в предпросмотре и доставке. Получатели — известные личные пользователи и все
  настроенные, не отключённые группы.
...
| `/cwl_forecast` | подключённая группа | прогноз составов текущего и оставшихся раундов ЛВК |
| `/cwl_forecast_schedule` | подключённая группа | кнопочная настройка расписания администратором |
| `/cancel` | личка | сброс текущего setup-state пользователя |

### Прогноз ЛВК

`/cwl_forecast` может запустить любой участник подключённой группы. Для каждого
активного tracked clan бот отправляет отдельную матрицу: фактические составы
текущей войны и потенциальные составы оставшихся соперников. Прогнозный состав
содержит сильнейших зарегистрированных в League Group игроков и не утверждает,
что именно они выйдут на будущую войну. Все колонки сортируются от высших ратуш к
низшим, а внизу матрицы показаны суммы уровней ратуш по колонкам.

Публичный CoC API возвращает `#0` вместо `warTag` будущих войн и не гарантирует
порядок соперников в массиве `clans`. Поэтому администратор один раз задаёт
неизвестные пары кнопками через `/cwl_forecast_schedule`. OCR, изображения и
неофициальные API не используются. Расписание переиспользуется между Telegram-
группами для того же сезона, состава League Group и нашего клана.

Premium emoji отправляются через Telegram entities. Если Telegram отклоняет
такое сообщение HTTP 400, бот ровно один раз отправляет обычный текстовый
fallback. Сетевые ошибки, 401/403/429 и 5xx fallback-retry не запускают.
