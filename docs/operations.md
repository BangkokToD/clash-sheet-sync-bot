...
Корневой `bot.py` — launcher:

```text
bot.py -> clash_sheet_sync_bot.bot.main()
```

Production-запуск остаётся прежним:
...
### 5.1. Самый безопасный backup при остановленном боте
...
### 5.2. Backup без остановки через sqlite `.backup`
...
`make check` должен проверять все tracked Python-файлы и тесты:
...
2. Проверить, что этот IP указан в Clash of Clans Developer Portal.
...
- CoC API вернул разные `season` для active tracked clans;
...
3. Временно отключить/удалить клан с другим season.
4. Повторить `/sync`.

## 14. Повреждённый `__bot_key`
...
4. Если проблема только в одной строке и user-values важны:
   - перенести user-values в корректную строку после sync;
...
Не выводить секреты в публичные чаты.
...
### 16.4. Проверить Python-зависимости
...
### 16.5. Проверить polling conflict

Если бот запущен дважды, Telegram может отдавать conflict polling.
